...
| 2026-09-14 | `changelog/proposals/2026-09-14-manga-narrative-db-corrections.md` | `dev-journal/2026-09-14-manga-narrative-db-corrections.md` | **Correction layer (HITL)** — full triangulation. New `corrections` table (NULL-meaning overrides + `is_deleted` tombstone) + `manga_db_edit.py` CLI (list/edit/delete/apply/corrections, annotated diff view). `manga_db_loader.py` auto-applies corrections after every upsert and re-syncs Chroma; `chroma_id()` centralized (counter-plan caught a false ID-discrepancy claim). 6 new tests → **117 total**. Live: `CIT4 → city` fixed on mingyun page 7, verified in SQLite + Chroma. |
...
| `manga_db_edit.py` | HITL correction CLI (list/edit/delete/apply/corrections — durable override layer + tombstone) | ~230 | 6 |
